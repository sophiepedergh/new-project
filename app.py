    ...
    if st.session_state.template_bytes is None:
        ...
        if st.button("🚀 Generate Bulk PDFs"):
            if not bulk_input_box.strip():
                ...
                with zipfile.ZipFile(zip_output_buffer, "w", zipfile.ZIP_DEFLATED) as archive:
                    for item_title in titles_array:
                        ...
                        if text_instances:
                            ...
                            # Define a wide, secure layout box spanning across page margins with expanded clearance
                            margin = 36
                            box_x0 = margin
                            ...
                            # Format text cleanly using HTML tags with a solid white background block
                            html_content = f"""
                        ...
                        # Format file name cleanly using the exact title text (keeping safe punctuation)
                ...
                st.success(f"Successfully generated {len(titles_array)} unique PDF files matching your titles!")
